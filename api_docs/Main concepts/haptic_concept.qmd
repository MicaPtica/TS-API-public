...
## Haptic, TENS, and EMS — stimulation modes of the Teslasuit

The Teslasuit electrodes deliver electrical stimulation, but the *effect* of that stimulation depends on how much current is applied. The same hardware supports two fundamentally different use cases:

### Haptic — sensory stimulation

At low amplitude settings the stimulation activates sensory nerve fibres just below the skin, producing a tactile sensation without causing muscle movement. In the context of the Teslasuit this is what the haptic subsystem delivers: a felt sensation that conveys information to the wearer.

**All 80 haptic channels across the suit can be used for haptic stimulation**

### EMS — motor stimulation

At higher amplitude settings the stimulation is strong enough to reach motor nerve fibres and cause an involuntary muscle contraction. This regime is called **EMS** (Electrical Muscle Stimulation) and is the basis of Functional Electrical Stimulation (FES) applications such as the [Elbow FES](../usecases/elbow_FES.qmd) and [Walking FES](../usecases/walking_FES.qmd) use cases.

**EMS can only be applied through channels that are positioned over a muscle projection** — i.e. channels whose electrodes sit directly above the muscle belly or its motor point. Channels located over bones or other non-muscular tissue will not produce a useful contraction even at high amplitude, and stimulation there at EMS-level power should be avoided.

### Key difference: stimulation power

| | Haptic / TENS | EMS |
|---|---|---|
| Amplitude | Low | High |
| Target tissue | Sensory nerves | Motor nerves / muscles |
| Effect | Felt sensation (touch, buzz, tingle) | Muscle contraction |
| Channel restriction | All 80 channels | Muscle-projection channels only |
| Typical use | Immersive feedback, notifications, VR/AR | Rehabilitation, FES, assisted movement |

::: {.callout-warning}
⚠️ When using EMS-level amplitudes, always start low and increase gradually. Only activate channels that are confirmed to be over the target muscle.
Refer to the device safety documentation before running any EMS protocol.
:::
...
### Calibration and amplitude scaling

Before any haptic API calls are made, the Teslasuit must be calibrated in the **Control Center**. Calibration helps to adjust to the individual sensitivity of each electrode channel on the current wearer and stores per-channel scaling factors.

**All amplitude values passed through the haptic API are relative to the calibrated range, not to absolute current levels.** An amplitude of `100` means 100% of the maximum current that was established during calibration for that channel — not 100% of the hardware maximum. An amplitude of `1` means the minimum perceptible level recorded during calibration.

This scaling is applied automatically and transparently to every `create_touch_parameters`, `play_touch`, and playable call. There is no API parameter or flag that bypasses or overrides it.

::: {.callout-note}
**This is a safety feature.** Calibration layer ensures that a given amplitude value produces a consistent, safe sensation. Skipping calibration before running stimulation scripts is not recommended — uncalibrated values may be stronger or weaker than expected.
:::
